...
/* timer_setup() callback wrapper: dci_drain_data() is also called
 * directly elsewhere with a dummy argument, so keep its signature and
 * just adapt it here for the timer. */
static void dci_drain_timer_cb(struct timer_list *t)
{
	dci_drain_data(0);
}
...
{
	struct dci_channel_status_t *status = from_timer(status, t, wait_time);
	int index = status - dci_channel_status;
...
int diag_dci_init(void)
{
	...
	timer_setup(&dci_drain_timer, dci_drain_timer_cb, 0);
	return DIAG_DCI_NO_ERROR;
err:
