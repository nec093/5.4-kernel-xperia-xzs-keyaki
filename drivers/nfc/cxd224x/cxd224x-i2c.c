...
struct cxd224x_dev {
	...
	/* xzs: RF reference clock - genuine Sony driver requires this to be
	 * enabled or the chip never generates a usable RF carrier, even though
	 * NCI/I2C communication works fine without it (confirmed via real
	 * hardware: protocol layer healthy, zero tag detections without this). */
	struct clk *felica_clk;
	/* xzs: genuine Sony driver applies these pinctrl states; our port
	 * never called pinctrl_select_state() at all, leaving the felica@29
	 * pins (irq_gpio/wake_gpio) on whatever pinmux config the bootloader
	 * left them in rather than the DTS-declared "felica_active" state. */
...
static int cxd224x_dev_open(struct inode *inode, struct file *filp)
{
	...
	if (call_enable) {
		/* xzs: this port's userspace HAL (Sony's real
		 * nfc_nci.cxd224x.msm8996.so, unlike the Essential Phone
		 * stack this driver was ported from) never issues an
		 * explicit CXDNFC_WAKE_CTL ioctl before writing - it expects
		 * the driver to power the chip (PON/wake_gpio) autonomously
		 * on open(), like the genuine stock driver for this board
		 * does. Without this, the first NCI write NACKs on the bus. */
...
static int cxd224x_dev_release(struct inode *inode, struct file *filp)
{
	...
	if (call_disable) {
		/* xzs: mirror of the auto-wake in cxd224x_dev_open(). */
                            ...
                            struct cxd224x_platform_data *pdata)
{
        ...
        /*nfc_int*/
	pdata->irq_gpio =  of_get_named_gpio_flags(dev->of_node, "cxd224x,irq_gpio", 0,NULL);
	if (pdata->irq_gpio < 0) {
	...
	pdata->wake_gpio = of_get_named_gpio_flags(dev->of_node, "cxd224x,wake_gpio", 0,NULL);
...
static int cxd224x_pinctrl_init(struct device *dev, struct cxd224x_dev *cxd224x_dev)
{
	...
	if (IS_ERR_OR_NULL(cxd224x_dev->pinctrl)) {
		dev_info(dev, "note devm_pinctrl_get() failed err:%ld\n",
...
static void cxd224x_pinctrl_select_state(struct device *dev, bool active)
{
	...
	if (!IS_ERR_OR_NULL(pins_state)) {
		int ret = pinctrl_select_state(cxd224x_dev->pinctrl, pins_state);
		if (ret)
			dev_err(dev, "error pinctrl_select_state(%s) err:%d\n",
				pins_state_name, ret);
	} else {
		dev_err(dev, "error pinctrl state-name:'%s' is not configured\n",
			pins_state_name);
	}
}
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (cxd224x_pinctrl_init(&client->dev, cxd224x_dev))
		dev_info(&client->dev, "pinctrl not available, continuing without it\n");
#ifdef CONFIG_WAKELOCK
	...
	/* xzs: genuine Sony driver enables this clock during probe(); our
	 * Essential Phone-derived port originally didn't reference it at all
	 * (and the DTS omitted the clocks property), which let probe()
	 * succeed but left the chip without its RF reference clock. */
	cxd224x_dev->felica_clk = clk_get(&client->dev, "felica_clk");
	if (IS_ERR(cxd224x_dev->felica_clk)) {
		dev_err(&client->dev, "Couldn't get felica_clk\n");
		ret = PTR_ERR(cxd224x_dev->felica_clk);
		goto err_clk_get;
	}
	ret = clk_prepare_enable(cxd224x_dev->felica_clk);
	if (ret) {
...
err_request_irq_failed:
	clk_disable_unprepare(cxd224x_dev->felica_clk);
err_clk_enable:
	clk_put(cxd224x_dev->felica_clk);
err_clk_get:
...
#if defined(CONFIG_OF)
static const struct of_device_id cxd224x_match_table[] = {
	{ .compatible = "sony,cxd224x", },
	{}
};
MODULE_DEVICE_TABLE(of, cxd224x_match_table);
#endif
