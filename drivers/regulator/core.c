 ...
 * serialized by regulator_nesting_mutex below) in place of raw
 * mutex_lock()/mutex_unlock() calls throughout this file. Ported
 * unmodified from pristine v5.4.302's core.c so the many CAF call
 * sites below (originally raw regulator_lock(rdev) /
 * regulator_unlock(rdev)) keep working against the now-ww_mutex
 * rdev->mutex declared in include/linux/regulator/driver.h.
 */
 ...
 *
 * CAF-specific (mainline replaced this with regulator_lock_dependent()'s
 * full ww_mutex acquire-context walk, not ported); reimplemented here on
 * top of the reentrant regulator_lock()/regulator_unlock() above instead
 * of raw mutex_lock_nested(), since rdev->mutex is now a ww_mutex.
 */
static void regulator_lock_supply(struct regulator_dev *rdev)
{
	for (; rdev; rdev = rdev_get_supply(rdev))
		regulator_lock(rdev);
}
