...
/*
 * The 4.14 block layer takes blk_status_t; the CAF mmc code passes errnos.
 */
#define mmc_blk_end_request(rq, err, n)	blk_end_request((rq), errno_to_blk_status(err), (n))
#define mmc_blk_end_request_all(rq, err)	blk_end_request_all((rq), errno_to_blk_status(err))
#define mmc_blk_end_request_all_locked(rq, err)	__blk_end_request_all((rq), errno_to_blk_status(err))
					...
					struct request *req)
{
	...
	if (!mmc_can_erase(card)) {
		err = -EOPNOTSUPP;
		mmc_blk_end_request(req, err, blk_rq_bytes(req));
		goto out;
	}
...
#endif
end_req:
	mmc_blk_end_request_all(req, ret);
			   ...
			   int ret)
{
	...
	if (mmc_card_sd(card)) {
		...
		if (!brq->data.fault_injected) {
			...
			if (blocks != (u32)-1)
				ret = mmc_blk_end_request(req, 0, blocks << 9);
		} else
			ret = mmc_blk_end_request(req, 0, brq->data.bytes_xfered);
	} else {
...
static int mmc_blk_end_packed_req(struct mmc_queue_req *mq_rq)
{
	...
	while (!list_empty(&packed->list)) {
		...
		list_del_init(&prq->queuelist);
		mmc_blk_end_request(prq, 0, blk_rq_bytes(prq));
		i++;
	}
...
static void mmc_blk_abort_packed_req(struct mmc_queue_req *mq_rq)
{
	...
	while (!list_empty(&packed->list)) {
		prq = list_entry_rq(packed->list.next);
		list_del_init(&prq->queuelist);
		mmc_blk_end_request(prq, -EIO, blk_rq_bytes(prq));
	}
...
void mmc_blk_cmdq_complete_rq(struct request *rq)
{
	...
	if (cmdq_req->cmdq_req_flags & DCMD) {
		...
		mmc_blk_end_request_all(rq, err);
		goto out;
	}
	/*
	 * In case of error, cmdq_req->data.bytes_xfered is set to 0.
	 * If we call mmc_blk_end_request() with nr_bytes as 0 then the request
	 * never gets completed. So in case of error, to complete a request
	 * with error we should use mmc_blk_end_request_all().
	 */
	...
	mmc_blk_end_request(rq, err, cmdq_req->data.bytes_xfered);
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	do {
		...
		case MMC_BLK_ECC_ERR:
			 ...
			 */
			ret = mmc_blk_end_request(req, -EIO,
						brq->data.blksz);
			if (!ret)
				goto start_new_req;
			break;
 ...
 start_new_req:
	if (rqc) {
		if (mmc_card_removed(card)) {
			...
			mmc_blk_end_request_all(rqc, -EIO);
		} else {
			/*
...
int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (ret) {
		...
		if (req) {
			mmc_blk_end_request_all(req, -EIO);
		}
		ret = 0;
		goto out;
	}
