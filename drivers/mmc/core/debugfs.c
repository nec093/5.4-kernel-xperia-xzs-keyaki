				...
				size_t cnt, loff_t *ppos)
{
	...
	if (cnt <= (strnlen_user(ubuf, cnt) + 1))
		goto exit;
	...
	tubuf_cnt = cnt - strnlen_user(ubuf, cnt) - 1;
	...
	if (strnlen_user(ubuf, cnt) < cnt - strlen(temp_ubuf))
		ret = copy_to_user((ubuf + strnlen_user(ubuf, cnt)),
				temp_ubuf, tubuf_cnt);
	else
		ret = -EFAULT;
	if (ret)
