...
/*
 * CAF raised this from mainline's 12 to 16 for boards needing more trip
 * points; kept at CAF's value since thermal_core.c below is CAF's own.
 */
...
/* upper limit requirement (CAF addition, not in mainline) */
...
/*
 * Max sensors that can be used for a single virtual thermalzone (CAF
 * addition, not in mainline).
 */
...
/*
 * use a high value for low temp tracking zone, to indicate an
 * invalid/uninitialized temperature (CAF addition, not in mainline).
 */
...
struct thermal_zone_device {
	...
	struct list_head thermal_instances;
	/*
	 * CAF uses an idr (not mainline's ida) for cooling-device ids;
	 * thermal_core.c below (CAF's own version) calls idr_init/idr_destroy
	 * and get_idr()/release_idr() against this field.
	 */
	struct idr idr;
	struct mutex lock;
	struct list_head node;
...
struct thermal_governor {
	...
	int min_state_throttle;	/* CAF addition (not in mainline). */
};
...
/*
 * CAF additions (not in mainline): virtual (multi-sensor-aggregating)
 * thermal zones.
 */
...
/*
 * CAF's own thermal_core.c defines these with a non-const `char *type`
 * (its old-style signature); the const-qualified mainline prototype
 * here does not match, so match CAF's instead.
 */
