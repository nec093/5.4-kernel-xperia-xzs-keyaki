				       ...
				       struct xt_action_param *par)
{
	...
	MT_DEBUG("qtaguid[%d]: iface_stat: %s(%s): "
		 ...
		 xt_hooknum(par), __func__, el_dev->name, el_dev->type,
		 xt_family(par), proto, direction);

	spin_lock_bh(&iface_stat_list_lock);
	entry = get_iface_entry(el_dev->name);
	if (entry == NULL) {
		IF_DEBUG("qtaguid[%d]: iface_stat: %s(%s): not tracked\n",
			 xt_hooknum(par), __func__, el_dev->name);
		spin_unlock_bh(&iface_stat_list_lock);
		return;
	}
				    ...
				    struct xt_action_param *par)
{
	...
	MT_DEBUG("qtaguid[%d]: find_sk(skb=%pK) family=%d\n",
		 xt_hooknum(par), skb, xt_family(par));
	...
	switch (xt_family(par)) {
	case NFPROTO_IPV6:
		sk = nf_sk_lookup_slow_v6(dev_net(skb->dev), skb, xt_in(par));
		break;
	case NFPROTO_IPV4:
		sk = nf_sk_lookup_slow_v4(dev_net(skb->dev), skb, xt_in(par));
		break;
	default:
		return NULL;
	}
	...
	if (sk) {
		MT_DEBUG("qtaguid[%d]: %pK->sk_proto=%u->sk_state=%d\n",
			 xt_hooknum(par), sk, sk->sk_protocol, sk->sk_state);
	}
	return sk;
}
...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	MT_DEBUG("qtaguid[%d]: entered skb=%p xt_in(par)=%p/out=%p fam=%d\n",
