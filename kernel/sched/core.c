...
/*
 * ---- CAF/Sony compatibility shims -------------------------------------
 * The 4.14 port currently runs the mainline CFS scheduler (no EAS, WALT,
 * schedtune or core isolation).  Vendor drivers still reference a few CAF
 * scheduler entry points; provide behaviour-preserving fallbacks so they
 * link.  TODO: port the CAF/EAS scheduler extensions properly.
 */
#ifdef CONFIG_HOTPLUG_CPU
...
/* Core isolation is not implemented: callers fall back to plain hotplug/DVFS */
...
unsigned int sched_get_cpu_util(int cpu)
{
	...
	unsigned long capacity = cpu_rq(cpu)->cpu_capacity_orig;

	util = min(util, capacity);
	return div64_ul((u64)util * 100, capacity);
}
